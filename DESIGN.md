...
- `WorkflowAvailabilitySnapshot` and `WorkflowPlanReadinessEvaluator` compare a plan only with detached exact adapter/ability IDs. Availability is not an authorization or capability grant: runtime callers must complete policy and capability checks before supplying it. Their readiness evidence binds workflow ID, revision, definition checksum, normalized input hash, and plan hash; it contains no raw workflow input, step arguments, provider output, secrets, or credentials.
