...
final class AbilityExecutionGateway {
	...
	private function write_permission_unblocks_tool( string $tool, array $auth, bool $is_intelligence_tool ): bool {
		if ( $this->is_lifecycle_write( $tool ) || in_array( $tool, array( 'site_editor.delete_record', 'navigation.delete_menu', 'media.delete_item', 'navigation.assign_location', 'content.restore_trashed', 'site_editor.update_record', 'site_editor.set_style', 'site_editor.restore_record', 'revisions.restore_content', 'navigation.update_item', 'content_fields.update_field', 'maintenance.clean_post_cache', 'maintenance.flush_rewrite_rules' ), true ) ) {
			return false;
		}
