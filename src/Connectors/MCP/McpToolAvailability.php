...
final class McpToolAvailability {
	...
	public function operations_manifest_for_user( int $user_id, ?AbilitiesRegistry $registry = null ): array {
		...
		return array(
			'description'        => 'Exact MCP operational tool names and availability for the connected WordPress user. Choose only available tools; blocked_by explains why an operation is unavailable (global_disabled, role_policy, role_default_read_only, missing_user, or missing_role) before assuming WordPress data or permissions are missing.',
			'policy'             => array(
