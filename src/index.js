...
const INTERNAL_LINK_STATE_LABELS = {
	all: 'All',
	...
	thin: 'Thin',
	stale: 'Stale',
	link_heavy: 'Link-heavy',
};
const INTERNAL_LINK_FLAG_LABELS = {
	orphan: 'Orphan',
	underlinked: 'Underlinked',
	thin: 'Thin',
	stale_index: 'Stale',
...
function InternalLinkFlag( { flag } ) {
	return (
		<span className={ `aculect-ai-companion-link-flag is-${ flag }` }>
			{ INTERNAL_LINK_FLAG_LABELS[ flag ] || flag }
		</span>
	);
}

function InternalLinksMetric( { label, value, tone = '' } ) {
	return (
		<div className={ `aculect-ai-companion-link-metric ${ tone }` }>
			<span>{ label }</span>
...
function InternalLinksDashboard( { data, internalLinksMap } ) {
	const payload =
		internalLinksMap && typeof internalLinksMap === 'object'
			? internalLinksMap
			: {};
	const items = Array.isArray( payload.items ) ? payload.items : EMPTY_ARRAY;
	...
	const filters =
		payload.filters && typeof payload.filters === 'object'
			...
			: {};
	const emptyState =
		payload.emptyState && typeof payload.emptyState === 'object'
			? payload.emptyState
			: {};
	const filterUrl = tabUrl( 'links-map', data.adminPageUrl );
	const stateValue = filters.state || 'needs_review';
	const postTypeValue = filters.post_type || '';
	const statusValue = filters.status || '';

	return (
		<div className="aculect-ai-companion-links-map">
			<section className="aculect-ai-companion-links-map-hero">
				<div>
					<span className="aculect-ai-companion-eyebrow">
						Content structure
					</span>
					...
					<p>
						Review indexed relationship signals, orphan pages,
						underlinked content, and stale rows before asking an
						assistant to propose link changes.
					</p>
				</div>
				<div className="aculect-ai-companion-links-map-hero__actions">
					<Button
						href={
							payload.refreshUrl ||
							tabUrl( 'diagnostics', data.adminPageUrl )
						}
						variant="secondary"
						icon={ chartBar }
					>
					...
					<Button
						href={
							payload.suggestionsUrl ||
							tabUrl( 'learning', data.adminPageUrl )
						}
						variant="primary"
						icon={ comment }
					>
			...
			{ Number( summary.staleIndexRows || 0 ) > 0 && (
				<Notice status="warning" isDismissible={ false }>
					{ Number( summary.staleIndexRows || 0 ).toLocaleString() }{ ' ' }
					indexed items are stale. Refresh the content index before
					relying on this map for large internal-link decisions.
				</Notice>
			) }

			<section
				className="aculect-ai-companion-links-metrics"
				aria-label="Internal link map counts"
			>
				<InternalLinksMetric
					label="Indexed"
					value={ summary.totalIndexed }
				/>
				<InternalLinksMetric
					label="Visible"
				...
				<InternalLinksMetric
					label="Orphan"
					value={ summary.orphan }
					tone="is-warning"
				/>
				<InternalLinksMetric
					label="Underlinked"
					value={ summary.underlinked }
					tone="is-info"
				/>
				<InternalLinksMetric
					label="Stale"
					value={ summary.stale || summary.staleIndexRows }
					tone="is-muted"
				/>
			</section>
			...
			<form
				method="get"
				action="admin.php"
				className="aculect-ai-companion-links-filters"
			>
				<input type="hidden" name="page" value="aculect-ai-companion" />
				<input type="hidden" name="tab" value="links-map" />
				<SelectControl
					label="Review state"
					name="links_state"
					value={ stateValue }
					__next40pxDefaultSize
					options={ Object.entries( INTERNAL_LINK_STATE_LABELS ).map(
						( [ value, label ] ) => ( { value, label } )
					) }
				/>
				<TextControl
					label="Post type"
					name="links_post_type"
					value={ postTypeValue }
					placeholder="Any"
					__next40pxDefaultSize
				/>
				...
				/>
				<TextControl
					label="Underlinked below"
					type="number"
					min="1"
					max="100"
					name="links_min_inbound"
					value={ String( filters.min_inbound_links || 2 ) }
					__next40pxDefaultSize
				/>
				<TextControl
					label="Thin below words"
					type="number"
					min="1"
					max="5000"
					name="links_thin_words"
					value={ String( filters.thin_word_count || 300 ) }
					__next40pxDefaultSize
				/>
				<div className="aculect-ai-companion-links-filter-actions">
					<Button type="submit" variant="primary">
						Filter
					</Button>
					<Button href={ filterUrl } variant="secondary">
						Reset
					</Button>
				</div>
			</form>
			...
			{ clusters.length > 0 && (
				<section
					className="aculect-ai-companion-links-clusters"
					aria-label="Internal link clusters"
				>
					{ clusters.map( ( cluster ) => (
						<article
							key={ cluster.id || cluster.label }
							className="aculect-ai-companion-links-cluster"
						>
							<div className="aculect-ai-companion-links-cluster__header">
								<div>
									<h3>{ cluster.label || 'Content' }</h3>
									<p>
										{ Number(
											cluster.total || 0
										).toLocaleString() }{ ' ' }
								...
								<span>
									{ Number(
										cluster.inboundTotal || 0
									).toLocaleString() }{ ' ' }
									in /{ ' ' }
									{ Number(
										cluster.outboundTotal || 0
									).toLocaleString() }{ ' ' }
									out
								</span>
							</div>
							<ul>
								{ ( Array.isArray( cluster.items )
									...
									<li key={ item.postId || item.title }>
										<span>
											{ item.title ||
												`Post ${ item.postId }` }
										</span>
										<small>
											{ Number( item.inbound || 0 ) } in /{ ' ' }
											{ Number( item.outbound || 0 ) } out
										</small>
									</li>
								) ) }
							</ul>
			...
			<section className="aculect-ai-companion-links-table-section">
				<div className="aculect-ai-companion-links-table-toolbar">
					<p>
						Showing page { payload.page || 1 } of{ ' ' }
						{ payload.totalPages || 1 }.
					</p>
					<strong>
						{ Number( payload.total || 0 ).toLocaleString() } rows
					</strong>
				</div>
				{ items.length > 0 ? (
					<InternalLinksTable items={ items } />
				) : (
					<EmptyState title={ emptyState.title || 'No rows found' }>
						{ emptyState.description ||
							'Try another filter or refresh the content index.' }
					</EmptyState>
				) }
				<div className="aculect-ai-companion-log-pagination">
					<Button
						href={ payload.prevUrl || undefined }
						variant="secondary"
						disabled={ ! payload.prevUrl }
						accessibleWhenDisabled
					>
						Previous
					</Button>
					<Button
						href={ payload.nextUrl || undefined }
						variant="secondary"
						disabled={ ! payload.nextUrl }
						accessibleWhenDisabled
					>
						Next
					</Button>
...
function InternalLinksTable( { items } ) {
	return (
		<div className="aculect-ai-companion-links-table-wrap">
			<table className="widefat striped aculect-ai-companion-links-table">
				<caption className="screen-reader-text">
					Indexed internal-link relationships
				</caption>
				<thead>
					<tr>
				...
				<tbody>
					{ items.map( ( item ) => {
						...
						return (
							<tr key={ item.post_id || item.id || item.title }>
								<th scope="row">
									<strong>
										{ item.title || 'Untitled' }
									</strong>
									<span>
										{ item.type || 'content' } #
										{ item.post_id || item.id || 0 }
									</span>
								</th>
								<td>
									<span className="aculect-ai-companion-links-status">
										{ item.status || 'unknown' }
									</span>
								</td>
								<td>
									<span>
										{ Number(
											item.inbound_internal_links || 0
										) }{ ' ' }
										inbound
									</span>
									<span>
										{ Number(
											item.outbound_internal_links || 0
										) }{ ' ' }
										outbound
									</span>
								</td>
								<td>
									<div className="aculect-ai-companion-link-flags">
										{ flags.length > 0 ? (
											flags.map( ( flag ) => (
												<InternalLinkFlag
													key={ flag }
								...
								<td>{ item.suggested_next_action || '-' }</td>
								<td>
									<div className="aculect-ai-companion-links-actions">
										{ item.editUrl && (
											<Button
												href={ item.editUrl }
												variant="secondary"
...
function SettingsApp() {
	...
	const internalLinksMap =
		data.internalLinksMap && typeof data.internalLinksMap === 'object'
			? data.internalLinksMap
			: {};
		...
		<div className="aculect-ai-companion-app-root">
			...
			<main className="aculect-ai-companion-tab-panel">
				{ ( () => {
					...
					if ( tab.name === 'links-map' ) {
						return (
							<InternalLinksDashboard
								data={ data }
								internalLinksMap={ internalLinksMap }
							/>
						);
					}
