...
final class IntelligenceRegistryTest extends TestCase {
	...
	public function test_intelligence_tools_are_claude_safe_and_scoped(): void {
		...
		$issue_reporter = $this->registry->module( 'plugin.issue.report' );
		self::assertNotNull( $issue_reporter );
		self::assertSame( 'Aculect Intelligence', $issue_reporter->group() );
		self::assertSame( array( 'content:read' ), $issue_reporter->required_scopes() );
		self::assertFalse( $issue_reporter->is_read_only() );
		self::assertSame( 'plugin.issue.report', $this->registry->internal_id( 'plugin_issue_report' ) );
	}
	...
	public function test_plugin_issue_report_prepares_public_github_issue_draft(): void {
		$result = $this->registry->execute(
			'plugin_issue_report',
			array(
		...
		self::assertSame( 'ready_for_client_submission', $result['status'] );
		...
		self::assertStringContainsString( 'Generated by the Aculect AI Companion MCP issue reporter', $result['body'] );
	...
	public function test_capability_directory_summarizes_abilities_workflows_and_intelligence(): void {
		...
		self::assertContains( 'plugin_issue_report', array_column( $summary['intelligence']['knowledge_tools'], 'tool' ) );
		...
		self::assertSame( 'plugin_issue_report', $summary['intelligence']['learning']['plugin_issue_tool'] );
