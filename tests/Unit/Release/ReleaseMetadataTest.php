...
final class ReleaseMetadataTest extends TestCase {
	...
	public function test_release_metadata_is_synchronized_for_current_version(): void {
		...
		self::assertSame( '0.5.0', $this->header( $plugin, 'Version' ) );
		self::assertStringContainsString( "define( 'ACULECT_AI_COMPANION_VERSION', '0.5.0' );", $plugin );
		self::assertSame( '0.5.0', $this->header( $readme, 'Stable tag' ) );
		self::assertSame( '0.5.0', (string) ( $package['version'] ?? '' ) );
