...
test('creates a Playground blueprint for a PR package URL', () => {
	const blueprint = createPreviewBlueprint({
		pluginUrl: 'https://uploads.github.example/aculect-ai-companion-pr-258.zip',
	});
...
test('rejects non-zip package URLs', () => {
	assert.throws(
		() =>
			createPreviewBlueprint({
				pluginUrl: 'https://uploads.github.example/aculect-ai-companion-pr-258.tar.gz',
			}),
		/\.zip/
	);
});
...
test('validates the WordPress.org listing blueprint without mutating it', () => {
	const blueprint = validateBlueprintFile('.wordpress-org/blueprints/blueprint.json');

	assert.equal(blueprint.landingPage, DEFAULT_LANDING_PAGE);
	assert.equal(blueprint.steps[0].pluginData.resource, 'wordpress.org/plugins');
});
